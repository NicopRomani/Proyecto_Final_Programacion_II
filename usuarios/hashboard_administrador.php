...
if (!$mensaje_exito) {
    ...
    if ($usuario_rol == 1 && !$busqueda) {
        $datos_usuarios = $conn->query("SELECT id, usuario, rol FROM usuarios")->fetchAll(PDO::FETCH_ASSOC);
    }
    if ($busqueda) {
        ...
        $motor_busquedaV = "
             ...
             ORDER BY v.id DESC
           ";
        $stmt_v = $conn->prepare($motor_busquedaV);
    ...
    <?php if ($mensaje_exito): ?>
        <div class="full-screen-message">
            <div class="alert success">
                <h3 style="margin-top: 0; font-weight: bold; font-size: 1.5em;">
                    Proceso:
                </h3>
                ...
                <a href="hashboard_administrador.php" class="btn primary" style="
                    background-color: #c0392b; 
                    color: white; 
                    text-decoration: none; 
                    padding: 10px 20px; 
    ...
    <div class="container">
        <div class="card">
            ...
            <div class="panel-header">
                <h2 class="titulo-principal">Panel administrador</h2>
                <div class="user-actions">
                    <span class="user-badge" title="<?= htmlspecialchars($_SESSION['usuario']) ?> (<?= $usuario_rol == 1 ? 'ADMINISTRADOR' : 'EMPLEADO' ?>)">
                ...
                <form method="GET" class="form form-centered">
                    ...
                    <div class="form-inline">
                        ...
                        <div class="btn-search-group">
                            ...
                            <a href="hashboard_administrador.php" class="btn btn-secondary">Limpiar</a>
                        </div>
                    </div>
            ...
            if (!isset($mensaje_exito)):
                if ($usuario_rol == 1 || ($mostrar_busqueda && !empty($datos_usuarios))):
            ?>
                    <h3 id="usuarios">Usuarios (<?= $busqueda ? 'Resultado de la busqueda' : 'Todos' ?>)</h3>
                    <table class="table">
                        <thead>
                            <tr>
                                <th>ID</th>
                        ...
                        <tbody>
                            <?php
                            if (empty($datos_usuarios) && $busqueda): ?>
                                <tr>
                                    <td colspan="4" style="text-align: center;">No se encontro usuarios para "<?= htmlspecialchars($busqueda) ?>"</td>
                                </tr>
                            <?php
                            endif;

                            foreach ($datos_usuarios as $u):
                                $texto_rol = $u['rol'] == 1 ? 'Admin' : 'Empleado';
                            ?>
                                <tr>
                                    <td><?= $u['id'] ?></td>
                                    ...
                                    <td>
                                        <?php if ($usuario_rol == 1): ?>
                                            <a class="btn action" href="editar_usuario.php?id=<?= $u['id'] ?>">Editar</a>
                                            <a class="btn action" href="eliminar_usuario.php?id=<?= $u['id'] ?>" onclick="return confirm('seguro que desea eliminar?');">Eliminar</a>
                                            <a class="btn action" href="cambiar_rol.php?id=<?= $u['id'] ?>">Designar rol</a>
